...
// Track open rooms with passwords and players
const rooms = {};
...
socket.on('joinRoom', (roomCode, enteredPassword) => {
  if (rooms[roomCode] && rooms[roomCode].password === enteredPassword) {
    if (!rooms[roomCode].isFull) {
      ...
      // Proceed with the usual join room logic
    } else {
      socket.emit('roomFull'); // Let the client know that the room is full
    }
  } else {
    socket.emit('invalidPassword'); // Invalid password notification
  }
});
...
socket.on('disconnect', () => {
  for (let roomCode in rooms) {
    ...
    if (room.players.length === 0 && room.host === socket.id) {
      delete rooms[roomCode]; // Remove the room if the host disconnects
    }
  }
});
